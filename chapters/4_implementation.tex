...
\item the program is compiled and uploaded either using the WinOLPC  tools via Roboguide or using the WinOLPC tools directly \cite{fanuchandling}. The WinOLPC tool are located in \mintinline{shell-session}{C:/Program File(x86)/FANUC/WinOLPC/bin} in the Windows operating system.
...
The \mintinline{text}{MoveL} function generates manufacturer specific code for linear movement. 
...
Parameters \mintinline{text}{ProgStart} method:
